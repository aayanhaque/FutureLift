...
const Login = () => {
  ...
  // Check if we should show the signup form based on URL parameter
  useEffect(() => {
    // Get the signup parameter from the URL
    const params = new URLSearchParams(location.search);
    const showSignup = params.get('signup') === 'true';
    
    // If the signup parameter is true, show the signup form
    if (showSignup) {
      setIsActive(true);
    }
  }, [location]);
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      alert(error.response?.data?.errors?.[0]?.msg || 'Login failed. Please check your credentials.');
    }
  };
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      const userData = {
        ...
        userType: 'jobseeker' // Default to jobseeker, can be changed in profile
      };
        ...
        >
          ...
          >
            ...
            <Box className="social-icons" sx={{ display: 'flex', margin: '20px 0' }}>
              <IconButton href="#" className="icon social-circle-icon" sx={{ 
                border: 'none', 
                borderRadius: '50%', 
                width: '50px', 
                height: '50px', 
                ...
                <i className="fab fa-google-plus-g" style={{ color: '#fff' }}></i>
              </IconButton>
              <IconButton href="#" className="icon social-circle-icon" sx={{ 
                border: 'none', 
                borderRadius: '50%', 
